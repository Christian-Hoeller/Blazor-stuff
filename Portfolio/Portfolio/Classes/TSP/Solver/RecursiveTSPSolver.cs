...
namespace Portfolio.Classes.TSP.Solver
{
    public class RecursiveTSPSolver:ITSPSolver
    {
        public List<int> Solve(IReadOnlyList<Location> cities)
        {
            List<int> visitedCities = new List<int>();
            visitedCities.Add(new Random().Next(0, visitedCities.Count));   //add the first index of the cities as the startindex
